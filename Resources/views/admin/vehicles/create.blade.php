...
@section('content-header')
    <h1>
        {{ trans('ivehicles::vehicles.title.create vehicle') }}
    </h1>
    <ol class="breadcrumb">
        ...
        <li class="active">{{ trans('ivehicles::vehicles.title.create vehicle') }}</li>
    </ol>
@stop

@section('content')
    <div class="row">
        {!! Form::open(['route' => ['admin.ivehicles.vehicle.store'], 'method' => 'post']) !!}
                            ...
                            <div class="tab-content">
                                ...
                                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                    ...
                                    <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                                        @include('ivehicles::admin.vehicles.partials.create-fields', ['lang' => $locale])
                                    </div>
                                @endforeach
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-md-4 ">
                                    <div class="box box-primary">
                                        ...
                                        <div class="box-body">
                                            <div class='form-group{{ $errors->has("mileage") ? ' has-error' : '' }}'>
                                                {!! Form::number("[mileage]", old("mileage"), ['class' => 'form-control', 'data-slug' => 'source', 'placeholder' => trans('ivehicles::vehicles.form.mileage')]) !!}
                                ...
                                <div class="col-xs-12 col-md-4 ">
                                    <div class="box box-primary">
                                        ...
                                        <div class="box-body">
                                            <div class='form-group{{ $errors->has("engine") ? ' has-error' : '' }}'>
                                                {!! Form::text("engine", old("engine"), ['class' => 'form-control', 'data-slug' => 'source', 'placeholder' => trans('iivehicles::vehicles.form.engine')]) !!}
                                ...
                                <div class="col-xs-12 col-md-4 ">
                                    <div class="box box-primary">
                                        ...
                                        <div class="box-body">
                                            <div class='form-group{{ $errors->has("price") ? ' has-error' : '' }}'>
                                                {!! Form::number("price", old("price"), ['class' => 'form-control', 'data-slug' => 'source', 'placeholder' => trans('iivehicles::vehicles.form.price')]) !!}
                                        ...
                                        <div class="box-body">
                                            <select name="owner_id" id="owner" class="form-control">
                                                @foreach ($owners as $owner)
                                                    <option value="{{$owner->id }}" {{$owner->id == old('owner_id') ? 'selected' : ''}}>{{$owner->present()->fullname()}}
                                                        - ({{$owner->email}})
                                                    </option>
                                                @endforeach
                                            </select><br>
                                                ...
                                                @foreach($fuel as $index=>$item)
                                                    <label class="radio" for="{{$item}}">
                                                        <input type="radio" id="fuel" name="fuel"
                                                               value="{{$index}}" {{ old('fuel',0) == $index ? 'checked' : '' }}>
                                                        {{$item}}
                                                    </label>
                                                @endforeach
                                            </div>
                                                ...
                                                @foreach($transmission as $index=>$item)
                                                    <label class="radio" for="{{$item}}">
                                                        <input type="radio" id="transmission" name="transmission"
                                                               value="{{$index}}" {{ old('transmission',0) == $index ? 'checked' : '' }}>
                                                        {{$item}}
                                                    </label>
                                                @endforeach
                                            </div>
                        ...
                        <div class="box-body ">
                            <div class="row">
                                <div class="col-xs-12 col-sm-6">
                                    ...
                                    <button type="button" data-toggle="modal" data-target="#modalGallery"
                                            class="form-control btn btn-primary btn-flat">{{ trans('ivehicles::vehicles.form.View Gallery') }}</button>
                                </div>
                                <div class="col-xs-12 col-sm-6">
                                    <label for="options[videos]"><strong>{{trans('ivehicles::vehicles.form.videos')}}</strong></label>
                                    <textarea id="options" class="form-control" name="options[videos]"
                                              rows="3">{{$vehicles->options->videos ?? ''}}</textarea>
                                </div>
                            </div>
                        ...
                        <div class="box-body">
                            <select name="brand_id" id="brand" class="form-control">
                                ...
                                @foreach ($brands as $brand)
                                    <option value="{{$brand->id }}" {{$brand->id == old('brand_id') ? 'selected' : ''}}>{{$brand->name}}
                                    </option>
                                @endforeach
                            </select><br>
                        </div>
                ...
                <div class="col-xs-12 ">
                    <div class="box box-primary">
                        <div class="box-header">
                            ...
                            <div class="box-body">
                                @include('ivehicles::admin.fields.image')
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xs-12">
                    <div class="box box-primary">
                        <div class="box-header">
                            ...
                            <div class="box-body">
                                @include('ivehicles::admin.fields.maps',['field'=>['name'=>'address', 'label'=>trans('ivehicles::vehicles.form.address')]])
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @php
            $gallery=str_random(4);
        @endphp
        <input type="hidden" id="gallery" name="gallery" value="{{$gallery}}">
        {!! Form::close() !!}

        @include('ivehicles::admin.fields.gallery',['rand'=>$gallery??'','field'=>['name'=>'gallery', 'label'=>trans('ivehicles::vehicles.form.gallery'),'route_upload'=>route('ivehicles.vehicles.gallery.store'),'route_delete'=>route('ivehicles.vehicles.gallery.delete'),'folder'=>'assets/ivehicles/place/gallery/','label_drag'=>trans('ivehicles::vehicles.form.drag')]])
    </div>
@stop
